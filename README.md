...
2026年2月8日開催の天下一キーボードわいわい会 Vol.10に向けて作成し、2026年10月3日開催の Vol.12 に向けてマルチサイズ対応・新テーマ拡張を行いました。
...
すべてのテーマで、編集パネルの **「📐 印刷サイズ」** から用途に応じたサイズを自由に選択できます。初期値は展示カードで扱いやすい写真L判です。
...
通常の展示用途では、**写真L判〜A5サイズ**を推奨します。プレビューは画面幅に合わせて拡大され、PCとタブレットではカードを大きく確認できます。画面上の拡大率はPNGの配置や実寸には影響しません。

印刷サイズを大きくしても、カードだけを広げて文字を相対的に小さくすることはありません。デザインの基準幅を固定したまま、選択サイズの縦横比とPNG解像度を変更します。A5ではL判と同じ構図をより大きく、高解像度で印刷できます。
...
すべてのテーマに共通のフリーコメント欄があります。展示物の補足、制作メモ、SNSアカウントなどを最大200文字まで入力できます。

- 初期状態ではキーボードの概要説明が入力されています。内容は自由に編集・削除できます
- 空欄の場合はカードに表示されません
- 改行を保持します
- 初期設定の「自動」では、テーマ内の文字や情報パネルを避けて空き領域へ配置します
- 必要に応じて左上・右上・左下・右下へ固定できます
- 文字サイズは小・標準・大から選択できます
- テーマ固有の色とフォントを継承し、既存デザインをできるだけ崩さず重ねます

## プレビューとPNGの一致

PNGはプレビューに表示しているカード要素そのものから生成します。出力専用の別レイアウトは使用しないため、文字サイズ、改行、コメント位置、配色、テーマ固有の装飾はプレビューと同じになります。出力時に解除するのは画面確認用の拡大表示だけです。

## プレビュー上でのデザイン編集

すべてのテーマで、既存の編集対象テキストとフリーコメントをプレビュー上から直接調整できます。

- 要素をドラッグして移動
- 選択枠の四隅で縦横比を保った拡大縮小、上下・左右のハンドルで高さ・幅だけ変更
- 画像対応テーマでは、アップロードした画像も直接ドラッグ・拡大縮小
- 画像のカラー／モノクロをチェックで切り替え、プレビューとPNGへ同じ状態で反映
- 文字をダブルクリックして編集（`Ctrl+Enter` / `⌘+Enter` で確定、`Esc` で取消）
- 選択した要素を50〜200%で拡大縮小
- 「文字も一緒に拡大」のチェックを外すと、文字サイズを維持して箱だけ拡大縮小
- 移動量・倍率・拡大方式はJSONへ保存され、読み込み時に復元
- 選択枠や編集ガイドはPNGに含まれません
